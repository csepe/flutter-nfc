...
class MyAppState extends State<MyApp> with SingleTickerProviderStateMixin {
  ...
  NFCTag? _tag;
  String? _result, _writeResult, _resultText;
...
/*   Future<List<ndef.NDEFRecord>> readString() async {
  var record = null;
  var tag = await FlutterNfcKit.poll(timeout: const Duration(seconds: 10), iosMultipleTagMessage: "Multiple tags found!", iosAlertMessage: "Scan your tag");
  if (tag.ndefAvailable!) {
    record = await FlutterNfcKit.readNDEFRecords(cached: false);
    inspect(record);
  }
...
void writeNFC(String text) async {
  ...
  if (tag.ndefWritable!) {
    print('writable');
    await FlutterNfcKit.writeNDEFRecords([ndef.TextRecord(text: text)]);
  }
  await FlutterNfcKit.finish();
} */
            ...
            ElevatedButton(
              onPressed: () async {
                try {
                  ...
                  } else if (tag.type == NFCTagType.mifare_ultralight || tag.type == NFCTagType.mifare_classic || tag.type == NFCTagType.iso15693) {
                    var ndefRecords = await FlutterNfcKit.readNDEFRecords();
                    var ndefString = '';
                    for (int i = 0; i < ndefRecords.length; i++) {
                      ndefString += '${i + 1}: ${ndefRecords[i]}\n';
                    }
                    ndefString.split(" ").forEach((string) {
                      if (string.contains("text=")) {
                        _resultText = string.replaceAll("text=", "");
                      }
                    });
                    setState(() {
                      _result = ndefString;
                    ...
                    onPressed: () {
                      showDialog(
                          ...
                          builder: (BuildContext context) {
                            return SimpleDialog(title: const Text("Record Type"), children: <Widget>[
                              SimpleDialogOption(
